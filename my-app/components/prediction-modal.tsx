...
import { AlertTriangle, TrendingUp, Leaf, Loader2 } from "lucide-react"
...
export function PredictionModal({ open, onOpenChange, metrics }: PredictionModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-2xl">AI Climate Impact Prediction</DialogTitle>
          <DialogDescription>Based on your current city configuration and historical climate data</DialogDescription>
        </DialogHeader>

        {loading ? (
          <div className="py-12 flex flex-col items-center justify-center gap-4">
            <Loader2 className="w-8 h-8 animate-spin text-primary" />
            <p className="text-sm text-muted-foreground">Training AI model on climate data...</p>
          </div>
        ) : prediction ? (
          <div className="space-y-6 py-4">
            <div className="text-center">
              <div className="text-5xl font-bold text-primary mb-2">{prediction.temperature}°C</div>
              <p className="text-sm text-muted-foreground">Predicted temperature by 2030</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3 p-4 rounded-lg bg-muted/50">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Using TensorFlow.js trained on historical temperature data from New York, adjusted for your
                    city&apos;s pollution level of {metrics.pollution} units.
                  </p>
                </div>
              </div>
              ...
              >
                {prediction.impact > 1 ? (
                  <AlertTriangle
                    className={`w-5 h-5 mt-0.5 flex-shrink-0 ${prediction.impact > 3 ? "text-red-500" : "text-yellow-500"}`}
                  />
                ) : (
              ...
              <div className="text-xs text-muted-foreground text-center p-3 bg-muted/30 rounded-lg">
                This prediction uses a linear regression model trained on real climate data, adjusted by your
                city&apos;s pollution and green space metrics.
              </div>
            </div>
          </div>
        ) : null}
